...
const TenderAwarding = ({ tenderId, tenderStatus }) => {
  const { offers, loading, awarding, error, handleAward, awardedPO } =
    useTenderAwarding(tenderId);
  ...
  if (tenderStatus !== "Closed") {
    return (
      <div>
        Review of offers will be available after the submission deadline.
      </div>
    );
  }
          ...
          <tbody>
            {offers.map((offer) => (
              <tr key={offer.id}>
                ...
                <td>
                  {new Date(offer.submittedAt).toLocaleDateString("fr-FR")}
                </td>
                <td>
                  <button
                    onClick={() => handleAward(offer.id)}
                    disabled={awarding}
                  >
