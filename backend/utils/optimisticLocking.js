 ...
 */
async function updateWithRetry(table, id, updateFn, maxRetries = 3) {
  const pool = getPool();
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const result = await optimisticUpdate(table, id, record.version, updates);
...
const examples = {
  ...
  updateSupplierInfo: async (supplierId, clientVersion, updateData) => {
    const result = await optimisticUpdate('companies', supplierId, clientVersion, {
    ...
    if (!result.success) {
      ...
      const pool = getPool();
      const current = await pool.query('SELECT * FROM companies WHERE id = $1', [supplierId]);
      return {
        success: false,
        reason: 'VERSION_CONFLICT',
