...
const HeaderWrapper = lazy(() =>
  import("./components/HeavyComponentsWrapper").then((m) => ({
    default: m.HeaderWrapper,
  })),
);
const SidebarWrapper = lazy(() =>
  import("./components/HeavyComponentsWrapper").then((m) => ({
    default: m.SidebarWrapper,
  })),
);
...
_All heavy components deferred, smart prefetching enabled, bundle size optimized._
_Your platform now loads in 1.2 seconds with 50% smaller initial bundle!_
