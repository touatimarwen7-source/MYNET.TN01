...
export const useTenderForm = () => {
  ...
  const validateStep = useCallback(() => {
    ...
    if (currentStep === 0) {
      ...
      if (!formData.submission_deadline)
        errors.submission_deadline = 'La date limite de soumission est requise.';
      ...
      if (
        ...
        new Date(formData.opening_date) <= new Date(formData.submission_deadline)
      ) {
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      setFormErrors({
        general: 'Veuillez remplir toutes les sections requises avant de soumettre.',
      });
    ...
    console.log('🔄 [CreateTender] Submitting tender with data:', {
      ...
      criteria:
        formData.evaluation_criteria.reduce((sum, c) => sum + (Number(c.weight) || 0), 0) + '%',
    });
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || error.message || "Une erreur inattendue s'est produite.";
