...
const OfferAnalysisReport = ({ tenderId }) => {
  ...
  if (!report) {
    return (
      <Alert severity="info">
        Le rapport d'analyse n'est pas encore disponible pour cet appel
        d'offres.
      </Alert>
    );
  }
          ...
          <Card sx={{ backgroundColor: "success.light" }}>
            <CardContent>
              ...
              <Typography variant="h5" sx={{ fontWeight: "bold" }}>
                {summary.bestValue.supplierName}
              </Typography>
              <Chip
                icon={<StarIcon />}
                label="Recommandé"
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" sx={{ fontWeight: "bold" }}>
                {summary.lowestPrice.supplierName}
              </Typography>
              <Typography>
                {new Intl.NumberFormat("fr-TN", {
                  style: "currency",
                  currency: "TND",
                }).format(summary.lowestPrice.amount)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5" sx={{ fontWeight: "bold" }}>
                {summary.highestCompliance.supplierName}
              </Typography>
          ...
          <TableHead>
            <TableRow>
              ...
              {detailedComparison.suppliers.map((supplier) => (
                <TableCell key={supplier.id} align="right">
              ...
              <TableRow key={item.id}>
                ...
                {detailedComparison.suppliers.map((supplier) => (
                  <TableCell
                    ...
                    align="right"
                    sx={{
                      backgroundColor: item.prices[supplier.id]?.isLowest
                        ? "success.lighter"
                        : "inherit",
                    }}
                  >
                    {item.prices[supplier.id]
                      ? new Intl.NumberFormat("fr-TN", {
                          style: "currency",
                          currency: "TND",
                        }).format(item.prices[supplier.id].price)
                      : "N/A"}
                  </TableCell>
                ))}
              </TableRow>
