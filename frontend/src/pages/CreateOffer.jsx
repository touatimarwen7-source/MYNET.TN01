...
export default function CreateOffer() {
  ...
  const handleSelectFromCatalog = (product) => {
    ...
    newItems[selectedLineItemIndex].total_price =
      newItems[selectedLineItemIndex].unit_price * newItems[selectedLineItemIndex].quantity;
  ...
  const getTotalBidAmount = () => {
    // ✅ هذه الدالة يمكن أن تبقى هنا لأنها للعرض فقط
    ...
    <Box sx={{ backgroundColor: '#fafafa', paddingY: '40px' }}>
      <Container maxWidth="lg">
        <Button
          ...
          sx={{ marginBottom: '24px', color: institutionalTheme.palette.primary.main }}
        >
          Retour
        </Button>
        ...
        {isDeadlinePassed && (
          <Alert severity="error" sx={{ marginBottom: '24px' }}>
            ⏰ Cet appel d'offres est fermé. La date limite était:{' '}
        ...
        <Card sx={{ border: '1px solid #e0e0e0' }}>
          <CardContent sx={{ padding: '32px' }}>
            ...
            >
              {' '}
              {/* ✅ استخدام handleSubmit من الخطاف */}
              {step === 0 && (
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setOfferData({ ...offerData, supplier_ref_number: e.target.value })
                    }
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      setOfferData({ ...offerData, validity_period_days: parseInt(e.target.value) })
                    }
                    disabled={submitting || isDeadlinePassed}
                  />
                  ...
                  <FormControl fullWidth disabled={submitting || isDeadlinePassed}>
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setOfferData({ ...offerData, payment_terms: e.target.value })
                      }
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      setOfferData({ ...offerData, technical_proposal: e.target.value })
                    }
                    disabled={submitting || isDeadlinePassed}
                  />
                      ...
                      <TableBody>
                        {offerData.line_items.map((item, idx) => (
                          <TableRow key={idx} sx={{ borderBottom: '1px solid #e0e0e0' }}>
                            ...
                            <TableCell
                              align="right"
                              sx={{ color: institutionalTheme.palette.text.primary }}
                            >
                              {item.quantity}
                            </TableCell>
                  ...
                  >
                    <CardContent>
                      ...
                      <Stack spacing={1} sx={{ fontSize: '14px' }}>
                        <Typography>
                          📋 <strong>Total:</strong> {getTotalBidAmount()} TND
                        </Typography>
                        <Typography>
                          ⏱️ <strong>Validité:</strong> {offerData.validity_period_days} jours
                        </Typography>
                        <Typography>
                          💳 <strong>Paiement:</strong> {offerData.payment_terms}
                        </Typography>
                        <Typography>
                          📎 <strong>Documents:</strong> {offerData.attachments.length} fichier(s)
                        </Typography>
                      </Stack>
                    </CardContent>
                  </Card>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) =>
                          setOfferData({ ...offerData, commitment: e.target.checked })
                        }
                        disabled={submitting || isDeadlinePassed}
                      />
                    }
