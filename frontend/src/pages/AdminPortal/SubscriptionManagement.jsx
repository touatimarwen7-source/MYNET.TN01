...
export default function SubscriptionManagement() {
  const [plans] = useState([
    ...
    { id: 3, name: 'الخطة الاحترافية', price: 999, users: 'غير محدود', offers: 'غير محدود', features: 'الكل' },
  ]);

  const [subscriptions] = useState([
    { id: 1, company: 'شركة الأمل', plan: 'الخطة المتوسطة', active_users: 35, renewal_date: '2025-04-26' },
    { id: 2, company: 'شركة النجاح', plan: 'الخطة الأساسية', active_users: 8, renewal_date: '2025-02-15' },
  ]);
      ...
      <Container maxWidth="xl">
        <Stack spacing={3}>
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            ...
            <Button variant="contained" startIcon={<Add />}>خطة جديدة</Button>
          </Stack>
          ...
          <Grid xs={12} spacing={3} container>
            {plans.map((plan) => (
              <Grid xs={12} md={4} key={plan.id}>
                <Card sx={{ backgroundColor: '#FFFFFF', border: '2px solid #0056B3', borderRadius: '12px' }}>
                  <CardContent>
                    <Stack spacing={2}>
                      ...
                      <Typography variant="h4" sx={{ color: THEME.palette.primary.main, fontWeight: 700 }}>
                        د.ت {plan.price}
                        <Typography component="span" variant="body2" sx={{ fontWeight: 400 }}> / شهر</Typography>
                      </Typography>
                      <Stack spacing={1}>
                      ...
                      <Stack direction="row" spacing={1}>
                        <Button size="small" variant="outlined" startIcon={<Edit />}>تعديل</Button>
                        <Button size="small" variant="outlined" color="error" startIcon={<Delete />}>حذف</Button>
                      </Stack>
                    </Stack>
                  </CardContent>
                </Card>
                  ...
                  <TableBody>
                    {subscriptions.map((sub) => (
                      <TableRow key={sub.id}>
                        ...
                        <TableCell><Chip label={sub.plan} size="small" variant="outlined" /></TableCell>
