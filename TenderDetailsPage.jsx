...
const TenderDetailsPage = () => {
  ...
  const canShowAwardingInterface =
    isOwner && isDeadlinePassed && tender.status !== "Awarded";
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <p>
        ...
        {new Date(tender.deadline).toLocaleString("fr-FR")}
      </p>
      <hr />
      <p>{tender.description}</p>

      {/* --- Analysis Report Section --- */}
      {canShowAnalysisReport && <OfferAnalysisReport tenderId={tender.id} />}
      ...
      {/* --- Audit Log Section --- */}
      {isOwner && <TenderAuditLog tenderId={tender.id} />}
    </div>
  );
};
