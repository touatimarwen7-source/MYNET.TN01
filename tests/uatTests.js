...
const results = {
  categories: {
    linguistic: { passed: 0, total: 0, name: "Linguistique" },
    aesthetic: { passed: 0, total: 0, name: "Esthétique" },
    functional: { passed: 0, total: 0, name: "Fonctionnalité" },
    security: { passed: 0, total: 0, name: "Sécurité" },
  },
  details: [],
};
...
function scanSourceFiles() {
  ...
  try {
    ...
    pages.forEach((page) => {
      ...
      frenchCount += (
        content.match(
          /\b(Connexion|Inscription|Appel|Offre|Notification)\b/g,
        ) || []
      ).length;
    });
  } catch (e) {}
...
function checkDesignStructure() {
  ...
  try {
    const luxuryCSS = fs.readFileSync(
      path.join(styleDir, "luxuryDesign.css"),
      "utf8",
    );
    testCase(
      "aesthetic",
      "Glassmorphism",
      luxuryCSS.includes("backdrop-filter"),
    );
...
function runUAT() {
  ...
  Object.entries(results.categories).forEach(([key, category]) => {
    if (category.total > 0) {
      ...
      log(
        `\n${category.name}: ${category.passed}/${category.total} (${percentage}%)`,
        "cyan",
      );
    }
  });

  const totalTests = Object.values(results.categories).reduce(
    (s, c) => s + c.total,
    0,
  );
  ...
  log("\n" + "=".repeat(50), "cyan");
  log(
    `Score Global: ${totalPassed}/${totalTests} (${globalPercentage}%)`,
    "cyan",
  );

  if (globalPercentage >= 90) {
    log("✓ PLATEFORME PRÊTE POUR PRODUCTION", "green");
  } else if (globalPercentage >= 70) {
    log("⚠ PLATEFORME PRÊTE AVEC RÉSERVES", "yellow");
  } else {
