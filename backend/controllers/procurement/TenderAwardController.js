...
class TenderAwardController {
  ...
  async getAwardDetails(req, res) {
    try {
      ...
      const details = await TenderAwardService.getTenderAwardDetails(tenderId, req.user.id);
  ...
  async finalizeAward(req, res) {
    try {
      ...
      const result = await TenderAwardService.finalizeTenderAward(tenderId, req.user.id);
